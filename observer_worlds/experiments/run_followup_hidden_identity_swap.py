"""Follow-up Topic 2 — hidden identity swap experiment runner (Stage 1 skeleton).

Validates the CLI surface and writes ``config.json``. Stage 3 will
implement the candidate matching, hidden-fibre swap intervention, and
``hidden_identity_pull`` scoring.
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--matching-mode", type=str, default="nearest_neighbor",
                   choices=MATCHING_MODES,
                   help="How to find paired candidates A, B.")
    p.add_argument("--projection-tolerance", type=float, default=0.05,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72)
    print("Follow-up Topic 2: hidden identity swap — Stage 1 skeleton")
    print("=" * 72)
    print(f"  out             = {out}")
    ...
    print("Stage 1: no simulation or swap will run.")

    summary = (
        "# Follow-up Topic 2 — hidden identity swap (Stage 1 skeleton)\n\n"
        f"Generated: {datetime.now(timezone.utc).isoformat()}\n\n"
        "Stage 1 only validates the CLI and writes config.json. "
        "Stage 3 will implement the swap intervention.\n"
    )
